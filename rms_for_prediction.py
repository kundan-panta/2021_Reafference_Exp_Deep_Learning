...
# find index in rms_d_mean that's the closest to the rms of the cycle
rms_all_difference = np.reshape(rms_all, [1, rms_all.shape[0], rms_all.shape[1], rms_all.shape[2]]) - np.reshape(rms_d_mean, [rms_d_mean.shape[0], 1, 1, rms_d_mean.shape[1]])
rms_all_difference_norm = np.linalg.norm(rms_all_difference, axis=3)
rms_all_difference_least_index = np.argmin(rms_all_difference_norm, axis=0)

rms_all_difference_least_distance = np.zeros_like(rms_all_difference_least_index)
for i in range(rms_all_difference_least_index.shape[0]):
    for j in range(rms_all_difference_least_index.shape[1]):
        rms_all_difference_least_distance[i, j] = d_all_labels[rms_all_difference_least_index[i, j]]

yhat_mean = np.mean(rms_all_difference_least_distance, axis=1)
yhat_std = np.std(rms_all_difference_least_distance, axis=1)
