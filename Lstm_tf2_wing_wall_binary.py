...
d_train_labels = [[1] * 8 + [0] * 7] * 5 + [[1] * 8 + [0] * 5]
...
save_results = True
save_folder = root_folder + 'plots/2021.08.22_binary/'  # include trailing slash
save_filename = 'Ro={}_A={}_Tr={}_Te={}_d={}_in={}_bl={}_Nc={}_Ns={}_2L{}_lr={}'.format(
    Ro, A_star, ','.join(str(temp) for temp in sets_train), ','.join(str(temp) for temp in sets_test),
    ','.join(str(temp) for temp in d_train_labels[0]), ','.join(str(temp) for temp in inputs_ft),
    baseline_d, N_cycles_example, N_cycles_step, lstm_units, lr)
...
distances = np.zeros_like(labels)
...
for k in range(N_files_all):
    ...
    for i in range(N_examples):
        ...
        distances[k * N_examples + i] = file_d[k]
...
distances_train = distances[:N_files_train * N_examples_train]
X_test = data[N_files_train * N_examples_train:]
y_test = labels[N_files_train * N_examples_train:]
distances_test = distances[N_files_train * N_examples_train:]
...
yhat_test = keras.activations.sigmoid(np.squeeze(model_best.predict(X_test)))
yhat_train = keras.activations.sigmoid(np.squeeze(model_best.predict(X_train)))

# print model predictions
print("Predictions (Test):")
for p, prediction in enumerate(yhat_test):
    print('{:.2f}\t'.format(prediction), end='')
