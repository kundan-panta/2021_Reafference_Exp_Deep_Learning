...
for j in range(N_inputs):
    ...
    fig.data[len(d_all_labels)].visible = True
    ...
    for i in range(len(fig.data) // 2):
        step = dict(
            ...
            args=[{"visible": [False] * len(fig.data)},
                  ...
                  #   {"value": str(d_all_labels[i])},
                  ],
            label=str(d_all_labels[i])
        )
        step["args"][0]["visible"][i] = True  # Toggle i'th trace to "visible"
        step["args"][0]["visible"][i + len(d_all_labels)] = True  # Toggle i'th trace to "visible"
    ...
    fig.update_yaxes(range=(data_min[j], data_max[j]))

    fig.write_html(save_folder + save_filename + '/plot_input_' + str(j) + '.html')
    fig.show()
